...
def simulate_loss_harvesting(realized_pnl_df, holdings_with_pnl, user_type='개인'):
    ...
    # 종목별 절세 효과 계산 (각 종목을 팔았을 때 절감되는 세금)
    if not loss_candidates.empty and user_type == '개인':
        loss_candidates = loss_candidates.copy()
        # 종목별 한계 절세 = 종목 손실 × 22% (단순 산정, 공제 한도 안 넘은 경우)
        # 정확히는 공제 적용 후 차이지만, 종목별 직관적 표시를 위해
        loss_candidates['절세효과(원)'] = -loss_candidates['평가손익(원)'] * TAX_RATE
        loss_candidates['절세효과(원)'] = loss_candidates['절세효과(원)'].round()
